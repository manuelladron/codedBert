...
from codedbert_evaluator_parser import Evaluation_negpairs
import argparse
...
class codedbert_embedder(transformers.BertPreTrainedModel):
...
def get_embeddings(dataset, save_file, pretrained_model=None, random_patches=False):
    ...
    if pretrained_model != None:
        embedder = codedbert_embedder.from_pretrained(pretrained_model, output_hidden_states=True, return_dict=True)
    else:
        embedder = codedbert_embedder.from_pretrained('bert-base-uncased', output_hidden_states=True, return_dict=True)
    ...
    with torch.no_grad():
        for i, (patches, _, input_ids, attention_mask, _, _, img_name) in enumerate(tqdm(dataloader)):
            ...
            codedbert_embeds[img_name[0]] = {'text': seq,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Get embeddings codedbert')
    ...
    parser.add_argument('--save_file_name', help='Name for file with embeddings', default='codedbert_embeddings.json')
